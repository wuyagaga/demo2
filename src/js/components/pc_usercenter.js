...
export default class PCUserCenter extends React.Component {
	...
	componentDidMount(){
		...
		fetch("http://newsapi.gugujiankong.com/Handler.ashx?action=getuc&userid=222" , myFetchOptions)
			.then(response=>response.json())
			.then(json=>{this.setState({usercollection:json});
			});//通过localStorage.userid获取收藏信息，

		fetch("http://newsapi.gugujiankong.com/Handler.ashx?action=getusercomments&userid=222" , myFetchOptions)
			.then(response=>response.json())
			.then(json=>{
				this.setState({usercomments:json})
			});//通过localStorage.userid获取评论
			// console.log(this.state.usercomments.Title);
	}
	...
	render(){
		...
		? usercollection.map((uc,index) => (
			<Card key={index} title={uc.title} extra={<a target="_blank" href={`/#/details/${uc.uniquekey}`}>查看</a>}>
				<p>{uc.Title}</p>
			</Card>
			))
		: "还没收藏新闻..."
